...
function Button({ handleClick, value,disabled,title }: ButtonProps) {
    let classbtn = 'btn'
    let innerValue:any = value ;
    switch (value) { 
        case "CONFIRM": innerValue = <Right />; classbtn = 'btn-right';  break;
        case "REJECT": innerValue = <Wrong />; classbtn = 'btn-wrong'; break;
        case "IGNORE": innerValue = <Ignore />; classbtn = 'btn-ignore'; break;
        case "UNDO": innerValue = <Undo />; classbtn = 'btn-undo'; break;
    }
  return <button style={{ border: 'none', cursor: 'pointer' }}
    disabled={disabled}
    title={title}
    className={classbtn+' '+'btn'}    onClick={handleClick}>{innerValue}</button>;
}
